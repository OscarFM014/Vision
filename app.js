...
var visualRecognition = new VisualRecognitionV3({
  version: '2018-03-19',
  iam_apikey: 'mUor-UTg6955F5vgKdNZRXSMPITYWzEC7r6AQeYEkwBu'
});
...
function checkFileType(file, cb){
  ...
  if(mimetype && extname){
    ...
    cb('Error: Seleccione solo imágenes');
  }
}
  ...
  upload(req, res, (err) => {
    if(err){
      ...
      if(req.file == undefined){
        res.render('index', {
          msg: 'Error: Ningún archivo seleccionado'
        });
      } else {
        var images_file = fs.createReadStream(`./public/uploads/${req.file.filename}`);
        var classifier_ids = ["DefaultCustomModel_300192336"];
...
app.post('/upload2', (req, res) => {
  ...
  upload(req, res, (err) => {
    if(err){
      ...
      if(req.file == undefined){
        res.render('index', {
          msg: 'Error: Archivo no seleccionado'
        });
      } else {
        var images_file = fs.createReadStream(`./public/uploads/${req.file.filename}`);
